...
(defun leaf-p (node)
  "Checks to see if the node is a leaf or branch if it is a branch, then
   return the nil, otherwise return t."
...
(defun node-p (node)
  (if (eql (length node) 3)
      (if (and (or (typep (node-type node) 'keyword) ;; equates to (car node)
                   (typep (car (node-type node)) 'keyword))   
               (typep (node-range node) 'cons)     ;; equates to (cadr node)
               (or (typep (node-children node) 'cons)
                   (not (node-children node)))) ;; equates to (caddr node)
          t
          nil)
      nil)) 
#|
...
(defun get-leaf (node &key go-right)
  ...
  (do ((lr-function 
         (if go-right #'cadr #'car)))
      ((and (node-p node) (leaf-p node)) node)
    (if (node-p node) 
        (setf node (node-children node))
        (setf node (funcall lr-function node))))) 

(defun get-leaf-one-step (node &key go-right)
  (let ((lr-function
          (if go-right #'cadr #'car)))
    (if (node-p node)
        (setf node (node-children node))
...
;; This code is to be executed to speed up development in the repl
(setf *lemon-current-parser* (register-language :c "/home/dunderscore/workspaces/git-repos/tree-sitter-langs/tree-sitter-c/libtree-sitter-c")) 
(defvar *test-tree* (lemon-tree-parse-file "/home/dunderscore/workspaces/git-repos/lem/extensions/lemon-tree/test-programs/c/test.c"))
